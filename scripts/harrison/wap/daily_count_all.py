...
while(are_files):
    i += 1
    yesterday = (dt.now() - datetime.timedelta(i)).date()
    yesterdays_files = []
    for file in processed_files: #yesterdays_files
        if "client" in file:
    ...
    are_files = (len(yesterdays_files) > 0)
    if not are_files:
        break
    ...
    errors = 0
    network = {}
    for file in yesterdays_files: #yesterdays_files
        try:
