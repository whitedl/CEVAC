...
def ingest_file(fname, dbc):
    """Import data from a wireless usage csv."""
    cursor = dbc.cursor()
    errorCount = 0

    with open(fname, "r") as csvfile:
        reader = csv.reader(csvfile)
        insert_sql = ("INSERT INTO  wireless_usage (r_id, time, assoc_count, "
                      "auth_count) VALUES (?,?,?,?)")
        select_sql = ("SELECT wu_key FROM wireless_usage WHERE r_id = ? AND "
                      "time = ?")
        check_routers_sql = ("SELECT TOP 1 r_key FROM routers WHERE name = ? "
                             "AND mac = ?")
        ...
        try:
            ...
            logging.error("Couldn't find 'Client Count' line in %s. "
        ...
        for row in reader:
            ...
            if len(row) != 5:
                if len(row) != 0 and row[0] != ' ':
                    logging.error("Row is not a valid length. File: %s, "
                                  "line: %s.", fname, reader.line_num)
                    errorCount += 1
                continue
            ...
            try:
                ...
                logging.error("Value Error in converting string to timestamp."
                              " File: %s, line: %s.", fname, reader.line_num)
                errorCount += 1
                continue

            # Check to see if data is already in db. Don't add duplicate rows.
        ...
        if(dupCount > 0):
            logging.warning("In %s, found %d entries that are already in the "
                            "database.", fname, dupCount)
...
def ingest_file_fail(fname, dbc):
    ...
    with open(fname, "r") as csvfile:
        reader = csv.reader(csvfile)
        insert_sql = ("INSERT INTO  CEVAC_WATT_WAP_HIST_RAW (time, name, ssid,"
                      " total_duration, predicted_occupancy, unique_users) "
                      "VALUES (?,?,?,?,?,?)")
...
def ingest_file_floor(fname, dbc, xref):
    ...
    with open(fname, "r") as csvfile:
        reader = csv.reader(csvfile)
        insert_sql = ("INSERT INTO  CEVAC_WATT_WAP_FLOOR_HIST_RAW "
...
if(not len(sys.argv) == 2):
    logging.exception("No config file specified. Please specify the config "
                      "file: python get_forecast.py <config_file_path>")
    logging.shutdown()
    exit()
...
for fname in file_list:
    ...
    try:
        ...
        if len(xref) > 0:
            ...
            logging.error(
                "Unable to read xref, append to CEVAC_WATT_WAP_HIST_FLOOR")
    except csv.Error:
        logging.error("Failed to read %s as a csv file.", fpath)
    except Exception as e:
        logging.error("Unexpected error while processing file '%s'", fpath)
