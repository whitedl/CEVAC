...
AND TABLE_NAME='CEVAC_ASC_TEMP_DAY'
AND TABLE_TYPE ='VIEW')
BEGIN
-- If the view exists, the code in this block will run.
DROP VIEW CEVAC_ASC_TEMP_DAY
END
GO


CREATE VIEW CEVAC_ASC_TEMP_DAY AS

SELECT
--	NOTE: Uncomment line below once XREF becomes available
--	xref.Alias as Alias,
	ps.PointSliceID as PointSliceID,
--	NOTE: Comment out line above once XREF becomes available
...
--	NOTE: uncomment below once XREF becomes available
--	INNER JOIN
--	dbo.CEVAC_ASC_TEMP_XREF as xref ON xref.PointSliceID = val.PointSliceID
