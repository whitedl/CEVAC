...
AND TABLE_NAME='CEVAC_WATT_POWER_LATEST'
AND TABLE_TYPE ='VIEW')
BEGIN
-- If the view exists, the code in this block will run.
DROP VIEW CEVAC_WATT_POWER_LATEST
END
GO


CREATE VIEW CEVAC_WATT_POWER_LATEST AS
(
	SELECT
	temp.Alias,
	-- NOTE: Uncomment the above line and comment out the next once XREF is available for ASC_TEMP
	--temp.PointSliceID,
	temp.UTCDateTime, temp.ActualValue, temp.Year, temp.Month, temp.Day FROM CEVAC_WATT_POWER_DAY AS temp
	INNER JOIN
	(
		SELECT
--		PointSliceID,
