...
AND TABLE_NAME='CEVAC_WATT_TEMP_LATEST'
AND TABLE_TYPE ='VIEW')
BEGIN
-- If the view exists, the code in this block will run.
DROP VIEW CEVAC_WATT_TEMP_LATEST
END
GO
...
(
	...
	--temp.PointSliceID,
	temp.UTCDateTime, temp.ActualValue, temp.Year, temp.Month, temp.Day FROM CEVAC_WATT_TEMP_DAY AS temp
	INNER JOIN
	(
		SELECT
--		PointSliceID,
