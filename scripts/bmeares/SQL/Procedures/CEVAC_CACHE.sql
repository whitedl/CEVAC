IF EXISTS(SELECT 1 FROM sys.procedures WHERE Name = 'CEVAC_CACHE') DROP PROCEDURE CEVAC_CACHE;
GO
SET ANSI_NULLS, QUOTED_IDENTIFIER ON;
GO
CREATE PROCEDURE CEVAC_CACHE
	@tables NVARCHAR(500)
AS
...
--		EXEC(@ExecSQL);
	...
	SET @select_query = 'SELECT * INTO ' + @name_CACHE + ' FROM ' + @name + ' ' + isnull(@where_subquery, '');
	EXEC(@select_query);
	DECLARE @cache_query NVARCHAR(50);

	SET @cache_query = 'INSERT INTO CEVAC_CACHE_RECORDS(table_name, update_time, row_count) VALUES (''' + @name +''', )';
